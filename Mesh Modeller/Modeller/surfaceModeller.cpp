...
void generateArrays() {
	totalVertices = subcurve.numCurvePoints * NUMBEROFSIDES; 
	vCoords = (double*)malloc(totalVertices * sizeof(double) * 3);
	for (int i = 0; i < totalVertices; i++) {
		int j = i * 3;
		vCoords[j] = varray[i].x;
		vCoords[j + 1] = varray[i].y;
		vCoords[j + 2] = varray[i].z;
	}

	totalQuads = (subcurve.numCurvePoints - 1) * NUMBEROFSIDES;
	indices = (unsigned int*)malloc(totalQuads * sizeof(unsigned int) * 4);
	for (int q = 0; q < totalQuads; q++) {
		for (int v = q * 4; v < q * 4 + 4; v++) {
			indices[v] = qarray[q].vertexIndex[v % 4];
		}
	}
	...
	for (int i = 0; i < totalVertices; i++) {
		int j = i * 3;
		vertexNormals[j] = varray[i].normal.x;
		vertexNormals[j + 1] = varray[i].normal.y;
		vertexNormals[j + 2] = varray[i].normal.z;
	}
}
...
void drawQuadsAsLines() 
{
	...
	if (drawNormals)
	{
		...
		for (int i = 0; i < totalVertices; i++) {
			...
			glVertex3f(v.x + v.normal.x, v.y + v.normal.y, v.z + v.normal.z);
			glEnd();
		}
...
void exportMesh() {
	...
	outFile.open("savedMesh.txt");
	outFile << totalVertices << "\n";
	outFile << totalQuads << "\n";
